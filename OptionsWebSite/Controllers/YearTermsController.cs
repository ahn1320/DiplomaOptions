    ...
    public class YearTermsController : Controller
    {
        ...
        public ActionResult Edit([Bind(Include = "YearTermId,Year,Term,IsDefault")] YearTerm yearTerm)
        {
            if (ModelState.IsValid)
            {
                ...
                //if change the selected one to IsDefault,
                if (yearTerm.IsDefault == true)
                {
                    //find in the database where the previous IsDefault was,
                    var previous = db.YearTerms.Where(c => c.IsDefault == true).FirstOrDefault();
                    //if there is IsDefault
                    if (previous != null)
                    {
                        //chane the previous IsDefault to false. 
